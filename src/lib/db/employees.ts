...
export async function listEmployees(): Promise<DbEmployee[]> {
  ...
  const [{ data: emps, error }, { data: structures }] = await Promise.all([
    supabase
      .from("employees")
      .select(
        "id, employee_code, name, email, entity_id, team_id, ju_sales_subtype, designation, status, joined_on, last_working_day, bank, account, account_title, cnic, city, entities(name), teams(name,kind)",
      )
  ...
  for (const s of structures ?? []) {
    ...
    salaryByEmp.set(s.employee_id, Number(gross) || 0);
  }
